...
balls = [{"name": "orange", "hsv": [11, 180, 234], "bgr": [68, 130, 234]},
         ...
         {"name": "yellow",  "hsv": [28, 173, 164], "bgr": [105, 110, 102]}
         ]

cur_ball_names = ["orange", "blue", "green", "yellow"]
...
while cam.isOpened():
    ...
    mask4, cY = findBall(frame, hsv_frame, balls[3]["bgr"], balls[3]["hsv"])
    ...
    if cO is not None and cB is not None and cG is not None and cY is not None:
        result = [{"name": "orange", "value": cO},
                  {"name": "blue", "value": cB},
                  {"name": "green", "value": cG},
                  {"name": "yellow", "value": cY}
                  ]
        result.sort(key=lambda c: c["value"][1])
        chunks = []
        for i in range(0, len(result), 2):
            chunks.append(result[i:i+2])

        for chun in chunks:
            chun.sort(key=lambda c: c["value"][0])
        ball_names = []
        for yr in chunks:
            for xr in yr:
                # print(xr["name"])
                ball_names.append(xr["name"])
        # print("\n")
